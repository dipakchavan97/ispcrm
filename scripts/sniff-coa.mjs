...
async function main() {
  ...
  console.log('1. Setting up sniffer on port 3799...');
  await client.executeCommands(config, [
    ['/tool/sniffer/set', '=port=3799', '=memory-limit=100', '=file-name='],
    ['/tool/sniffer/start']
  ]);

  console.log('2. Sending probe UDP packet from worker to 10.200.0.6:3799...');
  try {
    ...
    console.log('Client result note:', err.message);
  }
  ...
  const packets = res[1] || [];
  console.log(`Sniffer captured ${packets.length} packet(s):`);
