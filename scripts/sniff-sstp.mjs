...
async function main() {
  ...
  console.log('1. Setting up sniffer for UDP 3799...');
  await client.executeCommands(config, [
    ['/tool/sniffer/set', '=filter-port=3799', '=filter-ip-protocol=udp', '=file-name='],
    ['/tool/sniffer/start']
  ]);

  console.log('2. Sending test UDP packet to 10.200.0.6:3799 from worker...');
  const sock = dgram.createSocket('udp4');
  const probeData = Buffer.from('TEST_UDP_3799_PROBE');
  sock.send(probeData, 3799, '10.200.0.6', (err) => {
    if (err) console.error('Send error:', err);
    else console.log('Packet sent successfully via UDP.');
    sock.close();
  });

  await new Promise(r => setTimeout(r, 600));

  console.log('3. Stopping sniffer and collecting packets...');
  ...
  const packets = res[1] || [];
  console.log(`Sniffer captured ${packets.length} packet(s) on sstp-ispcrm:`);
  for (const p of packets) {
    console.log(`  ${p.time}s [${p.direction}] ${p['src-address']} -> ${p['dst-address']} (${p['ip-protocol'] || p.protocol})`);
  }
}
